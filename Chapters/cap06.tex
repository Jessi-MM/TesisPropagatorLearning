...
Una compuerta $\vec{f}$ llamada \textbf{forget}, decide qué información proveniente del tiempo anterior $t-1$ se mantiene y cual no, tomando en consideración también la entrada $\vec{x}_t$, aplicando la función sigma (es decir, $\vec{f}$ es una capa \emph{Sigmoid}):
